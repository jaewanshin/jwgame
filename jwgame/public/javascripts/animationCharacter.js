...
function Character(commonAssets, leftAssets, rightAssets) {
...
Character.prototype.readyAnimation = function() {
	//Clear Canvas  
	this.canvasContext.clearRect(0, 0, this.canvasSize.width, this.canvasSize.height);
	this.canvasContext.drawImage(this.leftAssets[0], 0, 100);
	this.canvasContext.drawImage(this.rightAssets[0], 700, 100);
	...
	setTimeout( function() {
		character.steadyAnimation();
	}, 1500);
}

Character.prototype.steadyAnimation = function() {
	//Clear Canvas  
	this.canvasContext.clearRect(0, 0, this.canvasSize.width, this.canvasSize.height);
	...
	setTimeout( function() {
		character.bangAnimation();
	}, Math.floor(Math.random() * 2000) + 1000);
}

Character.prototype.bangAnimation = function() {
	//Clear Canvas
	this.canvasContext.clearRect(0, 0, this.canvasSize.width, this.canvasSize.height);
	this.canvasContext.drawImage(this.leftAssets[1], 0, 100);
	this.canvasContext.drawImage(this.rightAssets[1], 700, 100);
...
Character.prototype.shotAnimation = function(winner) {
	//Clear Canvas
	this.canvasContext.clearRect(0, 0, this.canvasSize.width, this.canvasSize.height);
	if(winner === "left") {
	...
	setInterval( function() {
		character.finishAnimation(winner);
	}, 500);
}

var win = 2;
var lose = 5;
Character.prototype.finishAnimation = function(winner) {
	//Clear Canvas
	if(win == 5)
		return;
...
Character.prototype.right_bangAnimation = function(winner) {
	//Clear Canvas
	this.canvasContext.clearRect(0, 0, this.canvasSize.width, this.canvasSize.height);
	this.canvasContext.drawImage(this.leftAssets[2], 0, 100);
	this.canvasContext.drawImage(this.rightAssets[2], 700, 100);
...
function init() {
	document.onkeydown = function( e ) {
	    ...
	    if ( typeof keys[e.keyCode] != 'undefined' ) {
    		if(keys[e.keyCode] === "enter") {
				character.shotAnimation("right");
			} else {
				character.shotAnimation("left");
			}
	    }
	};
	...
	character = new Character(commonAssets, leftAssets, rightAssets);
	setTimeout( function() {
