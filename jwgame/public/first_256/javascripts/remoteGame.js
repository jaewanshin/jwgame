...
var userName;
var roomName;
var isStartNetworkPaly= false;
window.onload = function() {
	$('#gameMenu').append('<b> User Name </b> <input type=text id="userName">'); 
}
...
function connectServer() {
	...
	socket.on('roomlist', function(data) {
		if(isStartNetworkPaly)
			return;

		console.log("room list: ", data);
		$('#gameMenu').empty();
		$('#gameMenu').append('<b> Room Name </b> <input type=text id="roomName"> <button onclick = createRoom();>Create Room</button> <br>');
		for (var key in data) {
			console.log(' name=' + key + ' value=' + data[key]);
			if(data[key].length > 2) {
				continue;
			}
			$('#gameMenu').append('<button value = "' + key + '" onclick = joinRoom(this.value);>join_' + key + '_' + data[key] + '</button>');
		}
	});// new message
	...
	socket.on("playgame", function(data) {
		alert(data);
		$('#gameMenu').empty();
		$('#gameMenu').append('<button onclick = createRoom();>Ready</button> <br>');
		$('#gameMenu').append('<button onclick = createRoom();>Exit</button> <br>');
	});
}
...
DrawGame.prototype.drawNetworkGame = function(first_argument) {
